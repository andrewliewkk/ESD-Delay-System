...
def issue_voucher():
	...
	try:
	    ...
	    # qr image creation
	    voucher_accept_url = "http://192.168.43.187:8000/sq-delay/redeem/service-vouchers/redeem/" + vid # URL TO BE CHANGED
	    ...
	    retrieve_pax_url = "http://passenger:5003/passenger-search/" + passengerid
	    ...
	    font = ImageFont.truetype('vouchers/font/Helvetica.ttf', 25)
	    title = vid
	    ...
	    #draw issue date
	    font = ImageFont.truetype('vouchers/font/Helvetica.ttf', 18)
	    title = date
	    w,h = font.getsize(title)
	    draw.text((160,293), title ,(255,255,255),font=font)

	    #Draw title
	    font = ImageFont.truetype('vouchers/font/Helvetica.ttf', 60)
	    if voucher_type == 'Meal':
	        ...
	        title = "Hotel Voucher"
	        hid = data["hid"]
	        hotel_url = "http://service_recovery:5002/service-hotels/" + str(hid)
